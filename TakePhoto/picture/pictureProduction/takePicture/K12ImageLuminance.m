...
#define PIXEL_PARSE_THREAD_COUNT 800
    ...
    CGSize imageSize = CGSizeMake(CVPixelBufferGetWidth(imageBuffer), CVPixelBufferGetHeight(imageBuffer));//CVImageBufferGetDisplaySize(imageBuffer);
    ...
    __block double totalLuminance = 0.0;
    __block NSInteger maxPixelCount = CVPixelBufferGetDataSize(imageBuffer);
...
static double CMSampleBufferGetLuminance(CMSampleBufferRef sampleBuffer){
    if(sampleBuffer == NULL) return 1.0;
    CVImageBufferRef pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
    if(pixelBuffer == NULL) return 1.0;
    
    return CVImageBufferGetLuminance(pixelBuffer);
}

@implementation K12ImageLuminance : NSObject
+ (double)luminanceWithCMSampleBuffer:(CMSampleBufferRef)sampleBuffer{
    return CMSampleBufferGetLuminance(sampleBuffer);
}
+ (double)luminanceWithCVImageBuffer:(CVImageBufferRef)imageBuffer{
    return CVImageBufferGetLuminance(imageBuffer);
}
@end
